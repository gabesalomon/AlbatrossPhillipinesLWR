...
# S. gracilis Dataset ----
Spratelloides_gracilis_restricted <- read_excel("AlbatrossPhillipinesLWR/Data/Matching_LWR_data/Matching_LWR_data.xlsx",4)
...
# Fishbase comparison ---
log10ab <- read_excel("AlbatrossPhillipinesLWR/Data/Matching_LWR_data/loga_b_comparison.xlsx",4)
...
ggplot(data = log10ab_after, aes(x=b, y=log10a, color=Source))+
  ...
  ggtitle("Length-Weight log10a vs b Comparison of S. gracilis")+
  xlab("b")+
  ylab("log10a")
...
ggplot(Spratelloides_gracilis_restricted, aes(x=SL_cm, y=Mass_g))+
  ...
  annotate("text" , label="y ~ 0.007467x^(2.988969)  RSE ~ 0.05461", x=4.75, y=0.325)+
  theme(axis.text.x = element_text(hjust = 0.5))+
  ggtitle("LWR of S. gracilis")+
  xlab("SL (cm)")+
  ylab("Mass (g)")
...
ggplot(rcf, aes(x=xS, y=Kn))+
  ...
  annotate("text" , label="Average Kn = 0.9958412", x=4.7, y=0.875)+  
  theme(axis.text.x = element_text(hjust = 0.5))+
...
S_gracilis_after <- read_excel("AlbatrossPhillipinesLWR/Data/One_month_LWR_data/One_month_LWR_data.xlsx",4)
