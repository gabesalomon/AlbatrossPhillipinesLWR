...
y <- c(S_fuscescens_after$Mass_g)
xS <- c(S_fuscescens_after$SL_cm)
xT <- c(S_fuscescens_after$TL_cm)
...
ggplot()+
  ...
  theme(axis.text.x = element_text(hjust = 0.5))+
  ggtitle("Length-Weight log10a vs b of S. fuscescens after 1 month in EtOH")+
  xlab("b")+
  ylab("log10a")
